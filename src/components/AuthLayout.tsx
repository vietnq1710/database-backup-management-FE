...
import { useMyPermissionsView } from "@/api/hooks";
import { DatabaseBackup, History, LayoutDashboard, LogOut, PanelLeft, ServerCog, ShieldCheck } from "lucide-react";
...
type MenuItem = {
  ...
  label: string;
  path: string;
  // mục chỉ dành cho admin / user có permission "permission:manage"
      ...
      <div className="flex items-center justify-center min-h-screen">
        <div className="flex flex-col items-center gap-8 p-8 max-w-md w-full">
          <div className="flex flex-col items-center gap-6">
            <h1 className="text-2xl font-black tracking-tight text-center uppercase">
...
function AuthLayoutContent({
  ...
  // mục "Phân quyền" chỉ hiện với admin / user có permission:manage
  const { canManage: canManagePermissions, resolving } = useMyPermissionsView();
  const visibleMenuItems = menuItems.filter(
    (item) =>
      !item.requiresPermissionManager || (!resolving && canManagePermissions),
  );
            ...
            <div className="flex items-center gap-3 px-2 transition-all w-full">
              ...
              {!isCollapsed ? (
                <div className="flex items-center gap-2 min-w-0">
                  <span className="text-[11px] font-black tracking-[0.25em] uppercase truncate text-[var(--panel-light)]">
                    Backup Manager
                  </span>
                </div>
              ) : null}
